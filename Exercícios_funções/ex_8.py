...
def calcula_pontos(marcados, sofridos):
    ...
    for i in range(len(marcados)):
        if marcados[i] > sofridos[i]:
            ...
            pontuacao += 1
    aprov = 100 * pontuacao / (len(marcados) * 3)
    return (pontuacao, aprov)

pontuacao, aprov = calcula_pontos(gols_marcados, gols_sofridos)
print(f'A pontuação do time foi de {pontuacao} e seu aproveitamento foi de {round(aprov, 2)}%')     